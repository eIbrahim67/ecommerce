    ...
    return (
        <div className="min-h-screen flex flex-col bg-background">
            ...
            <main className="flex-1 container mx-auto py-8">
                <h1 className="text-3xl font-bold mb-8" style={{ fontFamily: "'Quicksand', sans-serif" }}>Your Cart</h1>
                        ...
                        <div className="flex-1">
                            <div className="bg-card border border-border rounded-xl overflow-hidden">
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        {items.map((item) => (
                                            <tr key={`${item.product.id}-${item.selectedWeight || ''}`} className="border-b border-border last:border-0">
                                                <td className="p-4">
                                                    <div className="flex items-center gap-4">
                                                        <img src={item.product.image} alt={item.product.name} className="w-16 h-16 rounded-lg object-cover border border-border" />
                                                        <div>
                                                            <Link to={`/product/${item.product.id}`} className="font-bold hover:text-primary transition-colors line-clamp-1">
                                                                {item.product.name}
                                                            </Link>
                                                            {item.selectedWeight && <p className="text-xs text-text-body mt-1">Weight: {item.selectedWeight}</p>}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-4 font-semibold">${item.product.price.toFixed(2)}</td>
                                                <td className="p-4">
                                                    <div className="flex items-center border border-border rounded-lg w-max shrink-0">
                                                        <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-2 text-text-body hover:text-primary transition-colors"><Minus className="w-3 h-3" /></button>
                                                        <span className="w-8 text-center text-sm font-semibold">{item.quantity}</span>
                                                        <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-2 text-text-body hover:text-primary transition-colors"><Plus className="w-3 h-3" /></button>
                                                    </div>
                                                </td>
                                                <td className="p-4 font-bold text-primary">${(item.product.price * item.quantity).toFixed(2)}</td>
                                                <td className="p-4">
