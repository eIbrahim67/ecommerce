import Header from "@/components/Header";
import Footer from "@/components/Footer";
import NewsletterBanner from "@/components/NewsletterBanner";
import { MapPin } from "lucide-react";
import { useState } from "react";
import { api, unwrapResponse } from "@/lib/api";
import { toast } from "sonner";

const Contact = () => {
  const [formData, setFormData] = useState({
    name: "",
    email: "",
    phone: "",
    subject: "",
    message: ""
  });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const offices = [
    { title: "Office", address: "205 North Michigan Avenue, Suite 810", city: "Chicago, 60601, USA", phone: "(123) 456-7890", email: "contact@nestmart.com" },
    { title: "Studio", address: "205 North Michigan Avenue, Suite 810", city: "Chicago, 60601, USA", phone: "(123) 456-7890", email: "contact@nestmart.com" },
    { title: "Shop", address: "205 North Michigan Avenue, Suite 810", city: "Chicago, 60601, USA", phone: "(123) 456-7890", email: "contact@nestmart.com" },
  ];

  const helpItems = [
    { num: "01", title: "Visit Feedback", desc: "Let us know about your in-store experience." },
    { num: "02", title: "Employer Services", desc: "For corporate inquiries and bulk orders." },
    { num: "03", title: "Billing Inquiries", desc: "Questions related to payments and invoices." },
    { num: "04", title: "General Inquiries", desc: "Anything else you have on your mind." },
  ];

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.email || !formData.message) {
      return toast.error("Please fill in the required fields (Name, Email, Message).");
    }

    try {
      setIsSubmitting(true);
      const res = await api.post("/contact/submit", formData);
      unwrapResponse(res.data);
      toast.success("Your message has been sent successfully!");
      setFormData({ name: "", email: "", phone: "", subject: "", message: "" });
    } catch (error: any) {
      console.error("Failed to submit contact form:", error);
      toast.error(error.message || "Failed to send message. Please try again later.");
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="min-h-screen bg-background">
      <Header />
      {/* Help section */}
      <section className="container mx-auto px-4 xl:px-0 my-8">
        <p className="text-primary font-semibold mb-2">How can we help you?</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <h1 className="text-3xl font-bold mb-4" style={{ fontFamily: "'Quicksand', sans-serif" }}>Let us know how we can help you</h1>
            <p className="text-text-body text-sm leading-relaxed">We are always happy to hear from you. Feel free to contact us regarding any questions or concerns.</p>
          </div>
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {helpItems.map((item) => (
              <div key={item.num}>
                <h3 className="font-bold mb-2" style={{ fontFamily: "'Quicksand', sans-serif" }}>
                  <span className="text-primary">{item.num}.</span> {item.title}
                </h3>
                <p className="text-sm text-text-body">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Map placeholder */}
      <section className="container mx-auto px-4 xl:px-0 my-8">
        <div className="bg-surface-light rounded-xl h-64 flex items-center justify-center border border-border">
          <span className="text-text-body text-sm">🗺️ Map Placeholder</span>
        </div>
      </section>

      {/* Offices */}
      <section className="container mx-auto px-4 xl:px-0 my-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        {offices.map((office) => (
          <div key={office.title}>
            <h3 className="text-xl font-bold mb-3" style={{ fontFamily: "'Quicksand', sans-serif" }}>{office.title}</h3>
            <p className="text-sm text-text-body mb-1">{office.address}</p>
            <p className="text-sm text-text-body mb-1">{office.city}</p>
            <p className="text-sm text-text-body mb-1">Phone: {office.phone}</p>
            <p className="text-sm text-text-body mb-3">Email: {office.email}</p>
            <button className="bg-primary text-primary-foreground text-sm px-4 py-2 rounded-lg font-semibold flex items-center gap-2 hover:opacity-90 transition-opacity">
              <MapPin className="w-4 h-4" /> View map
            </button>
          </div>
        ))}
      </section>

      {/* Contact form */}
      <section className="container mx-auto px-4 xl:px-0 my-12">
        <p className="text-primary font-semibold mb-2">Contact form</p>
        <h2 className="text-3xl font-bold mb-6" style={{ fontFamily: "'Quicksand', sans-serif" }}>Drop Us a Line</h2>
        <form onSubmit={handleSubmit} className="max-w-2xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <input
              type="text"
              placeholder="Your Name *"
              required
              className="border border-border rounded-lg px-4 py-3 text-sm outline-none focus:border-primary transition-colors bg-card"
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
            />
            <input
              type="email"
              placeholder="Your Email *"
              required
              className="border border-border rounded-lg px-4 py-3 text-sm outline-none focus:border-primary transition-colors bg-card"
              value={formData.email}
              onChange={(e) => setFormData({ ...formData, email: e.target.value })}
            />
            <input
              type="tel"
              placeholder="Your Phone"
              className="border border-border rounded-lg px-4 py-3 text-sm outline-none focus:border-primary transition-colors bg-card"
              value={formData.phone}
              onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
            />
            <input
              type="text"
              placeholder="Subject"
              className="border border-border rounded-lg px-4 py-3 text-sm outline-none focus:border-primary transition-colors bg-card"
              value={formData.subject}
              onChange={(e) => setFormData({ ...formData, subject: e.target.value })}
            />
          </div>
          <textarea
            placeholder="Your message... *"
            required
            rows={5}
            className="w-full border border-border rounded-lg px-4 py-3 text-sm outline-none focus:border-primary transition-colors mb-4 bg-card"
            value={formData.message}
            onChange={(e) => setFormData({ ...formData, message: e.target.value })}
          />
          <button
            type="submit"
            disabled={isSubmitting}
            className="bg-primary hover:bg-brand-green text-primary-foreground px-6 py-3 rounded-lg font-semibold text-sm transition-colors disabled:opacity-50"
          >
            {isSubmitting ? "Sending..." : "Send message"}
          </button>
        </form>
      </section>

      <div className="container mx-auto px-4 xl:px-0 mt-8 mb-8">
        <NewsletterBanner />
      </div>
      <Footer />
    </div>
  );
};

export default Contact;
