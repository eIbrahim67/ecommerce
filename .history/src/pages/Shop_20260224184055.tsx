...
const Shop = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Breadcrumbs customSegments={selectedCategory ? { shop: "Shop", [selectedCategory.toLowerCase()]: selectedCategory } : undefined} />

      {/* Category Links replacing static breadcrumb links */}
      <section className="bg-surface-banner -mt-6">
        <div className="container mx-auto pb-8">
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => setSelectedCategory(null)}
              className={`text-sm px-4 py-1.5 rounded-full transition-colors ${!selectedCategory ? "bg-primary text-primary-foreground border-primary" : "bg-background border border-border hover:border-primary hover:text-primary"}`}
            >
              All
            </button>
            {filterTags.map((tag) => (
              <button
                ...
                className={`text-sm px-4 py-1.5 rounded-full transition-colors ${selectedCategory === tag ? "bg-primary text-primary-foreground border-primary" : "bg-background border border-border hover:border-primary hover:text-primary"}`}
              >
                {tag}
              </button>
            ))}
