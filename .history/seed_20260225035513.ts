...
async function seed() {
    ...
    try {
        ...
        console.error("Register failed:", e?.response?.status, e?.response?.config?.url, e?.response?.data);
        try {
            ...
            console.error("Login failed:", err?.response?.status, err?.response?.config?.url, err?.response?.data);
        }
    }
